...
<!-- Form chỉnh sửa -->
<div class="edit-flag-form">
    <form method="POST" action="<?php echo BASE_URL; ?>/flags/edit/<?php echo $flag['id']; ?>" enctype="multipart/form-data">
        <div class="form-grid">
            <!-- Thử thách -->
            <div class="mb-3">
                ...
                <select class="form-control" id="challenge_id" name="challenge_id" required>
                    ...
                    <?php foreach ($challenges as $challenge): ?>
                        <option value="<?php echo $challenge['id']; ?>" <?php echo $challenge['id'] == $flag['challenge_id'] ? 'selected' : ''; ?>>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="flag_name" name="flag_name" value="<?php echo htmlspecialchars($flag['flag_name']); ?>" placeholder="Nhập tên flag..." required>
            </div>

            <!-- Giá trị Flag -->
            <div class="mb-3">
                <label for="flag_value" class="form-label">Giá trị Flag</label>
                <input type="text" class="form-control" id="flag_value" name="flag_value" value="<?php echo htmlspecialchars($flag['flag_value']); ?>" placeholder="Nhập giá trị flag..." required>
            </div>

            <!-- Checkbox "Là hình ảnh" -->
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="is_image" name="is_image" <?php echo $flag['is_image'] ? 'checked' : ''; ?>>
                <label class="form-check-label" for="is_image">Là hình ảnh</label>
            </div>

            <!-- Hình ảnh (ẩn/mở theo checkbox) -->
            <div class="mb-3" id="image_path_field" style="display: <?php echo $flag['is_image'] ? 'block' : 'none'; ?>;">
                <label for="image_path" class="form-label">Hình ảnh</label>
                <?php if ($flag['is_image'] && $flag['image_path']): ?>
                    <p class="current-image">Hình ảnh hiện tại: <img src="<?php echo BASE_URL . '/' . $flag['image_path']; ?>" alt="Flag Image"></p>
                <?php endif; ?>
            ...
            <div class="mb-3 full-width">
                ...
                <textarea class="form-control" id="description" name="description" rows="4" placeholder="Nhập mô tả..."><?php echo htmlspecialchars($flag['description'] ?? ''); ?></textarea>
            </div>
